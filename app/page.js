...
import {
  ...
  Sun, Trees, Waves, Camera, Loader2, Check
} from 'lucide-react'
...
function PostEditor({ open, onClose, onSave, initial }) {
  ...
  const [images, setImages] = useState([]) // [{url, aspectRatio}]
  ...
  const submit = async () => {
    ...
    setSaving(true)
    try {
      await onSave({ title: title.trim() || 'Untitled Entry', content, mood, images })
      onClose()
          ...
          <div className="flex flex-wrap gap-2">
            {MOODS.map((m) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs border ${mood === m ? 'journal-chip-active' : 'journal-chip'}`}
              >{m}</button>
            ))}
          </div>
              ...
              {images.map((im, i) => (
                <div key={i} className="relative group rounded-xl overflow-hidden border" style={{ borderColor: 'rgba(128,128,128,0.2)' }}>
                  ...
                  <div className="absolute bottom-2 left-2 flex gap-1">
                    <button
                      onClick={() => setRatio(i, '16:9')}
                      className={`px-2 py-1 text-[10px] rounded-md ${im.aspectRatio === '16:9' ? 'bg-white text-black' : 'bg-black/60 text-white'}`}
                    >16:9</button>
                    <button
                      onClick={() => setRatio(i, '3:4')}
                      className={`px-2 py-1 text-[10px] rounded-md ${im.aspectRatio === '3:4' ? 'bg-white text-black' : 'bg-black/60 text-white'}`}
                    >3:4</button>
                  </div>
                </div>
              ))}
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              {SAMPLE_IMAGES.map((u) => (
                <button
                  key={u}
                  onClick={() => addSample(u)}
                  className="relative flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border hover:scale-105 transition-transform"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t" style={{ borderColor: 'inherit' }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={submit}
              disabled={saving}
              className="journal-btn-primary inline-flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium disabled:opacity-50"
            >
      ...
      <div className="p-6 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 text-xs journal-muted mb-2">
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 top-full mt-1 w-40 rounded-lg border shadow-lg z-10 overflow-hidden" style={{ background: 'var(--card-bg, #fff)', borderColor: 'rgba(128,128,128,0.2)' }}>
                <button
                ...
                ><Pencil className="w-3.5 h-3.5" /> Edit entry</button>
                <button
        ...
        {post.content && (
          <div className="journal-prose mt-2">{post.content}</div>
        )}
        ...
        <div className="mt-6 flex items-center gap-3 text-xs journal-muted">
          ...
          <span className="opacity-60">Kept privately in your journal</span>
        </div>
      </div>
    </article>
  )
            ...
            {THEMES.map(t => {
              ...
              return (
                <button
                  ...
                  className={`relative p-3 rounded-xl border text-left transition ${theme === t.id ? 'ring-2 ring-offset-2' : ''}`}
          ...
          <div className="grid grid-cols-2 gap-2">
            {FONTS.map(f => (
              <button
                ...
                className={`p-3 rounded-xl border text-left transition ${font === f.id ? 'ring-2 ring-offset-2' : ''}`}
                style={{
...
function App() {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('tani-font', font)
  }, [font])
  ...
  const savePost = async (data) => {
    if (editing) {
      const r = await fetch(`/api/posts/${editing.id}`, {
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'currentColor' }}>
              <BookOpen className="w-4.5 h-4.5" style={{ color: 'var(--bg-invert, #fff)', mixBlendMode: 'difference' }} />
            </div>
            <div>
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="journal-btn-primary inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-medium"
            >
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        {loading ? (
          <div className="flex items-center justify-center py-24 journal-muted">
            <Loader2 className="w-5 h-5 animate-spin mr-2" /> Loading your journal…
          </div>
        ) : posts.length === 0 ? (
          <EmptyState onStart={() => { setEditing(null); setEditorOpen(true) }} />
        ) : (
        ...
        <footer className="mt-20 pt-8 border-t text-center text-xs journal-muted" style={{ borderColor: 'rgba(128,128,128,0.15)' }}>
          <div className="flex items-center justify-center gap-1.5">
            ...
            <span>Written with intention · The Tani Journal</span>
          </div>
        </footer>
      </main>
      ...
      <button
        onClick={() => { setEditing(null); setEditorOpen(true) }}
        className="sm:hidden fixed bottom-6 right-6 w-14 h-14 rounded-full shadow-lg journal-btn-primary flex items-center justify-center z-20"
      >
...
function EmptyState({ onStart }) {
  return (
    <div className="text-center py-16 sm:py-24 fade-up">
      ...
      <h1 className="text-3xl sm:text-5xl font-semibold mb-3 leading-tight">Begin your story.</h1>
